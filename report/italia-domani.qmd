...
format:
  html:
    ...
    toc-depth: 4
    toc-expand: true
subtitle: "I primi dati da ReGIS"
execute:
  daemon: false
  freeze: false
...
![Esempio pagina dati ItaliaDomani](./imgs/dati-italia-domani.png){fig-alt="Esempio pagina dati ItaliaDomani" #fig-dati-italia-domani}
...
![Esempio metadati sui i dati aperti di ItaliaDomani](./imgs/metadati-italiadomani.png){fig-alt="Esempio metadati sui i dati aperti di ItaliaDomani" #fig-metadati-italia-domani}

## Note sui dati

A seguire alcune prime note su alcuni dei dati Regis pubblicati da ItaliaDomani.
...
display_markdown(f'''
È composto da {formatted_rows} righe e {columns} colonne e in @tbl-esempio è possibile vedere un paio di righe di esempio.<br>
Quindi, in questo dataset sono presenti i dati relativi a **{formatted_rows} progetti**.
''', raw=True)
...
**Altre misure** al contrario, come "Rivoluzione verde e transizione ecologica", "Digitalizzazione, innovazione, competitività e cultura" e "Istruzione e ricerca", sono **molto più frammentate**, con interventi più piccoli per lo più distribuiti su tutto il territorio (vedi [dati localizzazione](#universo-regis---localizzazione-del-pnrr)).
''', raw=True)
...
Interessante guardare a un po' tutte le **suddivisioni categoriche dei progetti**. Tra queste ad esempio quella dei `Sottosettori`: in @tbl-sottosettore vediamo i primi 10 per valore.<br>
...
#### Mappa progetti comunali

Nella **mappa** a seguire è possibile avere un riscontro visuale dell'ammontare dei finanziamenti: più è intenso il tono di colore, più è alto il valore di finanziamento PNRR per quel Comune. Al passaggio del mouse sopra un Comune, si può leggere il nome del Comune e il totale finanziato.

Il menu a tendina presente sopra la mappa, consente di visualizzare gli stessi dati, ma per missione. Ed è molto evidente ad esempio come i progetti di "Digitalizzazione, innovazione, competitività e cultura" coprano in qualche modo tutto il paese, mentre i progetti di "Istruzione e ricerca" hanno una copertura più a macchia di leopardo (in accordo con i numeri di @tbl-misure-comuni).

<iframe width="100%" height="759" frameborder="0"
  src="https://observablehq.com/embed/@aborruso/localizzazioni-pnrrr?cells=viewof+missione_select%2Cmappa_comuni"></iframe>

::: {.callout-tip}
## Incorporare questa mappa

Se la vuoi incorporare in una pagina web, puoi copiare il codice seguente e inglobarlo nel tuo sito:

```html
<iframe width="100%" height="759" frameborder="0"
  src="https://observablehq.com/embed/@aborruso/localizzazioni-pnrrr@98?cells=viewof+missione_select%2Cmappa_comuni"></iframe>
```
...
- encoding qual è?
- errore in uno dei campi (vedi "Stato CUP")
- mancanza di date
- specifiche metadati (DATS, SSTRING??)
- non sono nel catalogo dati nazionale
